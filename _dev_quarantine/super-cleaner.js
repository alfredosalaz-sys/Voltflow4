...
// Characters to clean (1-to-5 pass mappings)
const charsToClean = [
  'á', 'é', 'í', 'ó', 'ú', 'ñ', 'ü',
  'Á', 'É', 'Í', 'Ó', 'Ú', 'Ñ', 'Ü',
  '¿', '¡', 'º', 'ª',
...
charsToClean.forEach(cleanChar => {
  ...
  for (let pass = 1; pass <= 5; pass++) {
    ...
    if (current !== cleanChar) {
      replacementMap.push({ mojibake: current, clean: cleanChar });
    }
  }
});

// Custom replacements for mixed-level patterns
...
// Sort all replacements by length descending
...
files.forEach(file => {
  if (file.endsWith('.jpg') || file.endsWith('.png') || file.endsWith('.gif') || file.endsWith('.ico') || 
      ...
      file.includes('intelligent-cleaner.js')) {
    return;
  }
  ...
  try {
    ...
    uniqueReplacements.forEach(r => {
      const escaped = r.mojibake.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
    ...
    // Apply auto-decoder on any remaining non-ASCII blocks
    let autoCount = 0;
    content = content.replace(nonAsciiRegex, (match) => {
      const decoded = autoDecode(match);
      if (decoded !== match) {
        autoCount++;
        return decoded;
      }
    ...
    if (content !== originalContent) {
      ...
      console.log(`Cleaned file: ${path.relative(rootDir, file)} - Corrections: ${replacedCount}, Auto-decoded: ${autoCount}`);
    }
  } catch (err) {
...
console.log('Super cleanup completed.');
