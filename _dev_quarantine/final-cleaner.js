...
// Sort custom replacements by length descending
customReplacements.sort((a, b) => b.mojibake.length - a.mojibake.length);
...
files.forEach(file => {
  ...
  try {
    ...
    // Apply custom mixed-level replacements first
    customReplacements.forEach(r => {
      // Escape for regex
    ...
    // Apply generic auto-decoding for non-ASCII blocks
    let autoChanges = 0;
    ...
    if (content !== originalContent) {
      ...
      console.log(`Cleaned file: ${path.relative(rootDir, file)} - Custom fixes: ${customChanges}, Auto-decoded blocks: ${autoChanges}`);
    }
  } catch (err) {
...
console.log('Final encoding cleanup completed.');
